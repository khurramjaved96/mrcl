...
class CelebA_Linear(VisionDataset):
    """`Large-scale CelebFaces Attributes (CelebA) Dataset <http://mmlab.ie.cuhk.edu.hk/projects/CelebA.html>`_ Dataset.
    Args:
        root (string): Root directory where images are downloaded to.
        split (string): One of {'train', 'valid', 'test', 'all'}.
    ...
    def __init__(self, root, split="train", target_type="attr", transform=None,
                 target_transform=None, download=False, RLN=None):
        import pandas
        super(CelebA_Linear, self).__init__(root, transform=transform,
                                            target_transform=target_transform)
        self.split = split
        if isinstance(target_type, list):
            self.target_type = target_type
        ...
        self.attr = (self.attr + 1) // 2  # map from {-1, 1} to {0, 1}
        # self.attr_names = list(attr.columns)
        attrs_to_keep = ["Arched_Eyebrows", "Attractive", "Bangs", "Black_Hair", "Blond_Hair", "Heavy_Makeup", "Male", "Mouth_Slightly_Open", "No_Beard",
                         "Wavy_Hair", "Wearing_Hat", "Wearing_Lipstick"]

        self.indices = []
        for a in attrs_to_keep:
            self.indices.append(list(attr.columns).index(a))
    ...
    def __getitem__(self, index):
        ...
        if index in self.rep_cache:
            X = self.rep_cache[index]
        else:
            # print(self.filename[index])
            X = PIL.Image.open(os.path.join(self.root, self.base_folder, "img_align_celeba", self.filename[index]))
            if self.transform is not None:
            ...
            # print("Before RLN", X.shape)
            with torch.no_grad():
                X = self.RLN(X.unsqueeze(0), feature=True).cpu().detach().data.squeeze()

            # print("After RLN", X.shape)
            self.rep_cache[index] = X
            # print(len(CelebA.image_cache))

        target = []
        for t in self.target_type:
            if t == "attr":
                target.append(self.attr[index, :][self.indices])
