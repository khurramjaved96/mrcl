...
def main(args):
    ...
    config = mf.ModelFactory.get_model("na", "omniglot")
        ...
        if step % 600 == 0:
            ...
            for temp_temp in range(0, 40):
                ...
                if accs_avg is None:
                    ...
                    accs_avg += accs
            logger.info("Loss Test= %s", str(loss[-1].item()))
            writer.add_scalar('/metatest/tests/accuracy', accs_avg[-1]/40, step)
            logger.info('Test Set: step: %d \t Accuracy %s', step, str(accs_avg/40))

            accs_avg = None
            for temp_temp in range(0, 40):
                t1_test = np.random.choice(list(range(900)), args.tasks, replace=False)
                ...
                if accs_avg is None:
                    ...
                    accs_avg += accs
            logger.info("Loss Train = %s", str(loss[-1].item()))
            writer.add_scalar('/metatest/train/accuracy', accs_avg[-1] / 40, step)
            logger.info('Train Set: step: %d \t Accuracy %s', step, str(accs_avg / 40))
