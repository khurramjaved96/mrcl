...
def main(args):
    ...
    for step in range(args.steps):
        ...
        # print("Label before = ", y_step)
        y_step = y_step + class_increment
        # print("Label after = ", y_step)
    ...
    #     Meta-Learning
        ...
        if step > 100:
            if step%args.freq==0:
                maml.reset_TLN()
                support_set = buffer.sample_trajectory_random(100)
                for inner_temp in range(0,95,5):
                    temp_support_set = support_set[inner_temp:inner_temp+5]
                    x_spt, y_spt = [x[0] for x in temp_support_set], [x[1] for x in temp_support_set]
                    # print(y_spt)
                    temp_qry_set = random.choices(support_set[0:inner_temp], k=min(15,inner_temp))
                    temp_qry_set = support_set[inner_temp:inner_temp+15]
                    x_qry, y_qry = [x[0] for x in temp_qry_set], [x[1] for x in temp_qry_set]

                    # print(y_qry)

                    x_spt, y_spt = torch.stack(x_spt), torch.stack(y_spt)
                    x_qry, y_qry = torch.cat(x_qry).unsqueeze(0), torch.cat(y_qry).unsqueeze(0)

                    if torch.cuda.is_available():
                        x_spt, y_spt, x_qry, y_qry = x_spt.cuda(), y_spt.cuda(), x_qry.cuda(), y_qry.cuda()
                    # print(x_spt.shape, y_spt.shape, x_qry.shape, y_qry.shape)
                    accs, loss_meta = maml(x_spt, y_spt, x_qry, y_qry)
                    if meta_loss is None:
                        meta_loss = loss_meta[-1].item()
                    else:
                        meta_loss = meta_loss*0.99 + loss_meta[-1].item()*0.01
